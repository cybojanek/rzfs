...
Python script to generate test cases for `checksum::fletcher2::tests`.
...
for unpack_fmt in (">QQ", "<QQ"):
    ...
    for size in 16, 32, 64, 128:
        ...
        while idx < len(data):
            chunk = bytes(data[idx : idx + 16])
            idx += 16
            f = struct.unpack(unpack_fmt, chunk)[0:2]
